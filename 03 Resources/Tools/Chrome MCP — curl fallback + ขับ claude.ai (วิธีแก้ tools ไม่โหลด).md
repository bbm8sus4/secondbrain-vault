...
## chrome-mcp-doctor — auto-heal (เพิ่ม 2026-07-18)

พอร์ตตาย = ไม่ต้องทำอะไรเอง ระบบซ่อมเองแล้ว:

```bash
chrome-mcp-doctor            # เช็คทุกพอร์ต + เปิด Chrome โปรไฟล์ที่ดับให้เอง (รอ ≤30s)
chrome-mcp-doctor --status   # เช็คเฉยๆ ไม่ซ่อม
```

- map พอร์ต→โปรไฟล์: `~/.config/chrome-mcp/doctor.conf` (ตอนนี้ `12306=Profile 1`)
- **statusline row 6** โชว์ `⚙ Chrome MCP 12306 ✓/✗` ทุก harness
- **PreToolUse hook** (ทั้ง 4 harness): เรียก tool chrome-mcp ตอนพอร์ตตาย → hook เปิด Chrome ให้เองก่อน call วิ่ง — lazy heal, ไม่มี watchdog เปิด Chrome สุ่มๆ
- permissions allow ครบ: mcp tools + curl helper + doctor → ไม่มี prompt
- chrome-mcp + chrome-mcp-kuan ลงทะเบียนครบ 4 harness แล้ว → "tools not loaded" จบสำหรับ session ใหม่ (session เก่ายังใช้ curl fallback ตามคู่มือข้างบน)
- 12307 (โปรไฟล์ 2): ยังไม่มีโปรไฟล์ไหนลง extension จริง — ติดตั้งแล้วเปิดบรรทัดใน doctor.conf ก็ครบวงจร
