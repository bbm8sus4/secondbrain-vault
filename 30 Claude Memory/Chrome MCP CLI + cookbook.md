...
## v4 (2026-07-19): ปิด gap ที่เหลือ — โค้ดเป็น git repo แล้ว
**สำคัญ: toolkit เป็น git repo แล้ว** `~/chrome-mcp-jobs/.git` — `bin/chrome|chrome-learn|chrome-mcp-doctor|mcp-chrome-curl.py` เป็น source of truth ใน repo, **symlink → `~/bin`** (แก้ที่ไหนก็ได้). `out/ state/ LEARNINGS.jsonl` gitignored (customer PII). `./install.sh` ทำ symlink, `./test.sh` = 24 regression checks รันหลังแก้. commit ต้องระวัง gitignore ห้าม inline comment (เคย leak PII).
ของใหม่ใน CLI (verify แล้ว):
- **`chrome act <id> "<intent>"`** — NL action + **observe→cache→replay self-heal** ผ่าน local `claude -p` (Max sub ไม่ต้อง API key). cache `state/act_cache.json` keyed host+intent → รอบ 2 replay ~1s ไม่เรียก LLM.
- **`chrome extract-ai <id> "<what>" [--schema]`** — LLM extraction หน้ารก → validated JSON.
- **`chrome click/fill/key --frame <frameId>`** — iframe (frameId จาก `chrome read`; iframe els เข้าถึงผ่าน ref ได้ด้วย). actionability skip เมื่อ --frame (poll เป็น main-frame).
- **retry/backoff ใน `call()`** — transient fail retry 2 รอบ (0.5s,1s).
- **`smoke_test.py`** — engine จริงของ recipe B1: flow spec (open/click-text/fill/wait/gone/sleep), assert ผ่าน js-poll (bg tab ได้), screenshot-on-fail (base64→out/), Telegram. cron ได้.
gotcha ใหม่: `chrome js` คืน boolean เป็น **string 'true'** (CDP stringify); `chrome wait` อ่าน foreground tab เท่านั้น (bg ใช้ js-poll). ดู [[Chrome MCP learnings (usage-memory)|reference_chrome_mcp_learnings]] + `~/chrome-mcp-jobs/LIMITATIONS.md`.
