...
3. **Pin a model permanently** (e.g. Opus 4.6) — Prompt version + one-liner + caveats (settings.json wins over /model picker on restart; picker list is app-baked so can't add previous models). See [[Model config — Fable 5 available|reference_model_config_fix]].
...
Key teaching point: distinguish built-in commands from custom skills/plugins (`/Chrome-MCP-Owly`, `/CRUD-basic`, `/codex:*` are NOT built-in). Complements [[Anthropic Claude 101 curriculum (real)|reference_anthropic_claude_101]]. Vault not auto-pushed — `git push` secondbrain-vault manually if sharing.
