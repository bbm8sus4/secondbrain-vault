...
- **Accessibility permission + ad-hoc signing gotcha (2026-07-04)**: Warp keystroke needs Model Shift in System Settings → Accessibility. TCC keys the grant to the app's cdhash; ad-hoc signing means **every rebuild changes the cdhash → grant breaks → must re-grant**. So: only rebuild when necessary, and after granting you MUST relaunch the app (permission is cached per running instance; a full macOS reboot clears it cleanly). Bundle id `com.milindsoni.modelshift`; clean reset = `tccutil reset Accessibility com.milindsoni.modelshift`. Added Model Shift to Login Items (System Events) so it auto-opens after reboot. Verified 2026-07-04: installed asar == source (no pending rebuild), cdhash 2f0b1bad…, so the grant survives restart.
