...
**LIVE URL:** https://office-seating-plan.pages.dev (Cloudflare Pages, deployed 2026-07-01). Deploy: copy html→`~/Desktop/.office-deploy/index.html` แล้ว `wrangler pages deploy ~/Desktop/.office-deploy --project-name office-seating-plan --branch main --commit-dirty true`. แชร์ลิงก์ stable นี้ (ไม่ใช่ลิงก์ hash). localStorage เป็น per-origin → เว็บ deploy เริ่มจาก DEFAULT seed (= ข้อมูลจริง rooms 203-209 + คน); ถ้าแก้ใน local ต้อง Export→Import ไปที่เว็บ.
