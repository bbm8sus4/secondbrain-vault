...
- Keep the build script alongside the output so numbers/scope can be regenerated. Example lives at `~/SecondBrain/01 Projects/AI-101 for บ.ขอนแก่นอิเล็คทริค/Files/Costs/AI-Workshop-Quotation-build_docx.py`.

Reference layout that the user likes ("จัดเจน/ชัดเจน"): the Cotactic 5-page service-contract format — parties block (both Tax IDs), scope as counted deliverables, totals box, KPI/outcomes table, payment schedule with dates, transfer details, dual signatures. Related: [[project_seatmap]] (Thunder brand), [[reference_html_dark_purple_design]].
