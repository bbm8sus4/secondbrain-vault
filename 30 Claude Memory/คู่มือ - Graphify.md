...
**นำเข้า Obsidian vault แล้ว (2026-07-17):** `~/SecondBrain/_KnowledgeGraphs/{EasySlip,Thunder,BoostSMS,_Merged-CrossBrand}/` — 729 notes + 4 graph.canvas + `หน้าหลัก.md` + `สำรวจกราฟ.base`. Workspace `~/Code/secondbrain-graphs/` ยังเป็น source of truth (มี `clean_boostsms_logos.py` + `merge_crossbrand.py`). **กติกา re-sync: copy แค่ `*.md` + `graph.canvas` — อย่า copy `.obsidian/` ของกราฟ (จะทับ setting vault หลักพัง).** โฟลเดอร์ `_` ขึ้นต้น แยกจาก PARA. 729 ไฟล์เข้า auto-sync + git push ทุกรอบ (ผู้ใช้เลือกเอง แม้ขัดกติกา "graph นอก vault" เดิม; ถ้าอยาก sync-แต่-ไม่-push ใส่ `_KnowledgeGraphs/` ใน vault `.gitignore`).

**Obsidian enhancements (2026-07-17):** (1) ตัด logo noise — BoostSMS-01..08 design PNG = 50 nodes ซ้ำ ตัดออกด้วย `clean_boostsms_logos.py` (BoostSMS 237→187, merged 387→337). ต้อง `to_json(...force=True)` เพราะ shrink-guard บล็อก. (2) `สำรวจกราฟ.base` — Bases 4 views (rationale 124 ไฟล์ = ทำไมถึงตัดสินใจ / concept / document / by-community). tag confidence (EXTRACTED/INFERRED/AMBIGUOUS) อยู่บน**เส้น**ไม่ใช่ note — filter AMBIGUOUS ที่ระดับ note ไม่ได้ (0 hit). (3) `.obsidian/graph.json` เพิ่ม 4 color group `path:_KnowledgeGraphs/<แบรนด์>` (วางก่อน group เดิม). **Vault สุขภาพดีอยู่แล้ว:** `.gitignore` คุม `.smart-env` (189M embeddings) + workspace, repo 61M. plugins: smart-connections (เดียว) + core bases/canvas/graph/templates. schema debt: note จริง 469 มี type แค่ 159/tags 158 (~⅓) — เติม frontmatter = งานใหญ่รอบหน้า.
