...
- 2026-07-19 — Chrome MCP as pentest tool: session-inherited API replay (Burp Repeater without cert install via `chrome fetch`), userscript XHR/fetch hooks as passive proxy (fetchhook.user.js), authz matrix testing via `chrome fetch --headers` (swap tokens in single profile), network_capture for endpoint discovery, overlay-guard verification (click-jacking prevention testing), built 8 security/CTF recipes in `/chrome-usecases` skill.
