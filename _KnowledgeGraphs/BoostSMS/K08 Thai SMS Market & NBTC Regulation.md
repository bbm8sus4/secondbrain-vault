---
source_file: "_knowledge/K08-Thai-SMS-Market-NBTC.md"
type: "document"
community: "BoostSMS Knowledge Base Index"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/BoostSMS_Knowledge_Base_Index
---

# K08 Thai SMS Market & NBTC Regulation

## Connections
- [[AIS (Advanced Info Service)]] - `references` [EXTRACTED]
- [[BoostSMS Knowledge Base Index]] - `references` [EXTRACTED]
- [[BoostSMS Packages & Pricing (doc)]] - `references` [EXTRACTED]
- [[BoostSMS Platform]] - `references` [EXTRACTED]
- [[Grey Route]] - `references` [EXTRACTED]
- [[K01 SMS Marketing Fundamentals]] - `references` [EXTRACTED]
- [[K07 Timing & Frequency Best Practices]] - `references` [EXTRACTED]
- [[K09 Sender Name  Sender ID Thailand]] - `references` [EXTRACTED]
- [[K11 Thai SMS Case Studies]] - `references` [EXTRACTED]
- [[K12 Competitor Landscape (Thai + Global)]] - `references` [EXTRACTED]
- [[K13 Deliverability & DLR]] - `references` [EXTRACTED]
- [[K14 Compliance & Anti-Spam (PDPA, NBTC, Opt-inout)]] - `references` [EXTRACTED]
- [[NBTC (กสทช.)_1]] - `references` [EXTRACTED]
- [[PDPA (Thai Personal Data Protection Act)]] - `references` [EXTRACTED]
- [[Thaibulksms]] - `references` [EXTRACTED]
- [[TrueMove H (True Corporation)]] - `references` [EXTRACTED]
- [[Twilio]] - `references` [EXTRACTED]
- [[dtac (merged with True)]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/BoostSMS_Knowledge_Base_Index